...
**Golden Rule — “1 message \= all related operations.”** In one message, you must batch:
...
**Pragmatic exception — Chunked parallelism** If one blast would exceed token/tool limits, split into numbered chunks (e.g., **Chunk 1/3**). Each chunk must still batch *all* related operations within its scope.
...
* /src — application code  
  ...
  * /src/db/migrations — **Historical**, auto-generated database change scripts  
...
**Golden Rule — Never trust old migration files in /src/db/migrations for the current database structure.** They are historical records of changes, not a snapshot of the final result.

The definitive structure of the database is determined by applying all migration files in the /supabase/migrations/ folder in chronological order. These files are the authoritative history of every change.

When you need to understand the database schema, you MUST use the following file as the single source of truth:

* **The TypeScript Types (/src/lib/database.types.ts)**: This file is always generated from the local database after all migrations have been applied. It is the most accurate and up-to-date representation of the database structure that the application code should be using.

The **/supabase/schemas/initial_schema.sql** file is a snapshot used for initial setup and should not be considered the most current source of truth for daily development. **If you suspect the types file is out of sync, inform the user and recommend they run the database migration workflow.**
...
| senior-backend-engineer | Services/endpoints, migrations, tests | Code → /src/backend; Migrations → /src/db/migrations; Tests → /tests/backend | **Must follow the Golden Map workflow for all schema changes.** |
...
Coordination  
mcp\_\_ref\_\_agent\_spawn { type: "doc-analyzer" }  
mcp\_\_semgrep\_\_agent\_spawn { type: "security-scan" }  
mcp\_\_pieces\_\_agent\_spawn { type: "memory-coordinator" }  
mcp\_\_exasearch\_\_agent\_spawn { type: "searcher" }  
mcp\_\_playwright\_\_agent\_spawn { type: "ui-tester" }  
Execution  
